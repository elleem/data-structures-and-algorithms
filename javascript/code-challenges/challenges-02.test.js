...
const raisedToTheThird = (arr) => {
  let newArr = [];
  // returns a new array of each of those numbers raised to the 3rd power Math.pow()
...
const addQuestion = (arr) => {
  ...
  let questions = arr.map(function (n) {
    return n + "?";
  });
  return questions;
};
...
const forLoopTwoToThe = (arr) => {
  let squared = [];
  // use a for loop iterates over the array and
  for (let i = 0; i < arr.length; i++) {
    let n = arr[i];
    //T result of raising 2 to the power of the original input element.
    let numSquared = Math.pow(2, n);
    //returns a new array.
    squared.push(numSquared);
  }
